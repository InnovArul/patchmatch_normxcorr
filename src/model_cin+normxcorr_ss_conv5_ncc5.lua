...
function create_model()
    ...
    if(not opt.forceNewModel or lfs.attributes(LOAD_MODEL_NAME) ~= nil) then
        ...
        global_summary = nn.SpatialConvolution(nstates[2], nstates[3], 3, 3)(CCU_summary_relu):annotate{
        ...
        --reshape
        reshape = nn.Reshape(nstates[3]*5*5)(global_summary_maxpool):annotate{
            name='Reshaping unit',
            graphAttributes = {color = TEXTCOLOR, style = NODESTYLE, fillcolor = COLOR_AUGMENTS}
        };
    ...
    -- Ahmed paper branch
       -- Joining layer to join the filtered features from two subNetworks
        ...
        across_patch = nnpackage.SpatialConvolution(nstates[2], nstates[3], 3, 3)(patch_summary_relu):annotate{
        ...
        --reshape
        reshape = nn.Reshape(nstates[3]*5*5)(across_maxpool):annotate{
            name='Reshaping unit',
            graphAttributes = {color = TEXTCOLOR, style = NODESTYLE, fillcolor = COLOR_AUGMENTS}
        };
